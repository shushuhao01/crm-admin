  ...
  <div class="page-container">
    <el-card shadow="never" class="search-card">
      <el-form :model="searchForm" inline>
        <el-form-item label="租户名称/授权码">
          <el-input v-model="searchForm.keyword" placeholder="名称/编码/授权码" clearable style="width: 200px" />
        </el-form-item>
    ...
    <el-card shadow="never" class="table-card">
      <template #header>
        <div class="card-header">
          <span>租户列表</span>
          <el-button type="primary" @click="handleAdd"><el-icon><Plus /></el-icon>新增租户</el-button>
        </div>
      </template>
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="$router.push(`/tenants/detail/${row.id}`)">详情</el-button>
    ...
    <!-- 新增/编辑对话框 -->
    <el-dialog v-model="showDialog" :title="editingId ? '编辑租户' : '新增租户'" width="550px">
    ...
    <!-- 授权码生成成功提示 -->
    <el-dialog v-model="showLicenseDialog" title="租户创建成功" width="500px">
      <el-result icon="success" title="租户创建成功">
        <template #sub-title>
          <div class="license-result">
            <p>请将以下授权码发送给租户，用于登录系统：</p>
...
const form = reactive({
  name: '', code: '', packageId: '', maxUsers: 10,
  expireDate: null as Date | null, contact: '', phone: '', email: ''
})
...
const handleAdd = () => {
  ...
  Object.assign(form, { name: '', code: '', packageId: '', maxUsers: 10, expireDate: null, contact: '', phone: '', email: '' })
...
const handleEdit = (row: any) => {
  ...
  Object.assign(form, {
    name: row.name, code: row.code, packageId: row.package_id,
    maxUsers: row.max_users, expireDate: row.expire_date ? new Date(row.expire_date) : null,
  ...
  switch (cmd) {
    case 'regenerate':
      ElMessageBox.confirm('重新生成授权码后，原授权码将失效，租户需要使用新授权码登录。确定继续？', '重新生成授权码', { type: 'warning' })
        .then(async () => {
          const res = await request.post(`/admin/tenants/${row.id}/regenerate-license`)
          if (res.success) {
    ...
    case 'renew':
      ...
      break
    case 'suspend':
      ElMessageBox.confirm('暂停授权后，该租户下所有用户将无法登录。确定继续？', '暂停授权', { type: 'warning' })
        .then(async () => {
          const res = await request.post(`/admin/tenants/${row.id}/suspend`)
          if (res.success) { ElMessage.success('已暂停授权'); fetchData() }
        })
      break
    case 'resume':
      const res = await request.post(`/admin/tenants/${row.id}/resume`)
      if (res.success) { ElMessage.success('已恢复授权'); fetchData() }
      break
    case 'delete':
      ElMessageBox.confirm('删除租户将同时删除所有相关数据，此操作不可恢复！', '删除租户', { type: 'error' })
        .then(async () => {
          const res = await request.delete(`/admin/tenants/${row.id}`)
...
const handleRenew = async () => {
  ...
  try {
    const res = await request.post(`/admin/tenants/${renewingTenant.value.id}/renew`, {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      const res = await request.put(`/admin/tenants/${editingId.value}`, data)
      if (res.success) {
      ...
      const res = await request.post('/admin/tenants', data)
      if (res.success) {
        newLicenseKey.value = res.data.licenseKey
        showDialog.value = false
        showLicenseDialog.value = true
...
const fetchData = async () => {
  ...
  try {
    ...
    tableData.value = [
      { id: '1', name: '测试公司A', code: 'TESTA', license_key: 'TENANT-A1B2-C3D4-E5F6-G7H8', license_status: 'active', package_name: '企业版', user_count: 45, max_users: 100, expire_date: '2026-12-31', status: 'active', showFullKey: false },
      { id: '2', name: '测试公司B', code: 'TESTB', license_key: 'TENANT-X1Y2-Z3A4-B5C6-D7E8', license_status: 'pending', package_name: '专业版', user_count: 18, max_users: 50, expire_date: '2026-06-30', status: 'active', showFullKey: false }
    ]
    total.value = 2
  } finally {
    loading.value = false
